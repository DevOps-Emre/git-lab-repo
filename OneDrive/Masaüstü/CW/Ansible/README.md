...
- Connect to the control node via SSH and run the following commands.#(echo 'ClientAliveInterval 60' | sudo tee --append /etc/ssh/sshd_config)
(sudo service sshd restart)+(logout)-----> konsoldan çıkmayı engelleyen komutlar
...
--> scp -i yagmur.pem yagmur.pem ec2-user@ec2-54-157-152-173.compute-1.amazonaws.com:/home/ec2-user(pem dosyasını konsola kopyalama komutu!!!!)
```
